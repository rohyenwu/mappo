...
# Re-run only RL after restoring the original WiFi v9 local observation layout.
# BEB summaries are reused from the previous completed BEB evaluations.
#
# Run from the git repository root:
#   bash scripts/eval_wifi_v9_mbps_15cases_rerun_rl.sh
...
run_rl_duration() {
  ...
  echo "========== RL ${duration}s re-evaluation =========="
  for item in "${CASES[@]}"; do
    read -r mld sld <<< "${item}"
    exp_name="$(experiment_name rl "${duration}" "${mld}" "${sld}")"

    echo "[RL ${duration}s rerun] m${mld}_s${sld}"
    python -m onpolicy.scripts.eval.eval_wifi_v9_rl_mbps \
      ...
      --wandb_run_name "rl_15case_${TAG}_${duration}s_m${mld}_s${sld}_ep${EVAL_EPISODES}_rerun_obsfix" \
      --model_dir "${MODEL_DIR}"
  done
}
...
compare_duration() {
  local duration="$1"
  local output_dir="${RESULT_BASE}/wifi_v9_mbps_15case_compare_${duration}s"
  local compare_args=()
  ...
  python -m onpolicy.scripts.eval.compare_wifi_mbps_cases \
    ...
    --wandb_group "wifi_v9_15case_${TAG}_${duration}s" \
    --wandb_run_name "beb_vs_rl_15case_${TAG}_${duration}s_obsfix" \
    --wandb_entity "${WANDB_ENTITY}"
}
