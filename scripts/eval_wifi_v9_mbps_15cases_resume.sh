...
# Resume after RL 50s stopped at m20_s6.
# Run from the git repository root:
#   bash scripts/eval_wifi_v9_mbps_15cases_resume.sh
#
# Execution order:
#   1. RL 50 sec eval for m20_s6, m25_s6, m30_s6
#   2. BEB 10 sec eval for all 15 cases
#   3. BEB 50 sec eval for all 15 cases
#   4. BEB vs RL 10 sec plots
#   5. BEB vs RL 50 sec plots
...
run_rl_50_resume() {
  ...
  echo "========== RL ${duration}s resume evaluation =========="
  for item in "${RL_50_RESUME_CASES[@]}"; do
    read -r mld sld <<< "${item}"
    exp_name="$(experiment_name rl "${duration}" "${mld}" "${sld}")"

    echo "[RL ${duration}s resume] m${mld}_s${sld}"
...
compare_duration() {
  local duration="$1"
  local output_dir="scripts/eval_results/WiFi_v9/wifi_v9_mbps_15case_compare_${duration}s"
  local compare_args=()
  ...
  for item in "${ALL_CASES[@]}"; do
    ...
    compare_args+=(
      --case "${label}|scripts/eval_results/WiFi_v9/wifi_v9_beb_mbps/${beb_exp}/beb_mbps_summary.json|scripts/eval_results/WiFi_v9/wifi_v9_rl_mbps/${rl_exp}/rl_mbps_summary.json"
    )
  done
  ...
  python -m onpolicy.scripts.eval.compare_wifi_mbps_cases \
    ...
    --output_name "wifi_v9_beb_vs_rl_${duration}s_15case.png" \
