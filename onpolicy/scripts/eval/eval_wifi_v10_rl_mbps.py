#!/usr/bin/env python
"""Evaluate a trained WiFi v10 RL checkpoint with fixed-duration Mbps metrics."""
...
from onpolicy.envs.wifi_v10.wifi_env import WiFiEnvV10
...
def parse_args(args, parser):
    ...
    parser.add_argument("--wandb_project", type=str, default="WiFi_v10_eval_mbps")
    parser.add_argument("--wandb_group", type=str, default="compare_wifi_v10_mbps")
...
def main(args):
    ...
    run_dir = build_eval_run_dir(all_args, "wifi_v10_rl_mbps")
    run = init_wandb(all_args, run_dir, "wifi_v10_rl_mbps")
        ...
        while not accumulator.done():
            ...
            if bool(np.all(dones)):
                if accumulator.done():
                    break
                raise RuntimeError(
                    "WiFi v10 Mbps eval reached env done before the fixed-duration "
