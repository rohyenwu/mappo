#!/usr/bin/env python
"""Evaluate a trained WiFi v11 RL checkpoint with fixed-duration Mbps metrics."""
...
def parse_args(args, parser):
    ...
    parser.add_argument("--wandb_project", type=str, default="WiFi_v11_eval_mbps")
    parser.add_argument("--wandb_group", type=str, default="compare_wifi_v11_mbps")
...
def main(args):
    ...
    run_dir = build_eval_run_dir(all_args, "wifi_v11_rl_mbps")
    run = init_wandb(all_args, run_dir, "wifi_v11_rl_mbps")
    ...
    for episode in range(all_args.eval_episodes):
        ...
        while not accumulator.done():
            ...
            if live_interval > 0.0 and (
                ...
                print(
                    f"[RL Mbps Live] Episode {episode + 1}/{all_args.eval_episodes} | "
            ...
            if bool(np.all(dones)):
                if accumulator.done():
                    break
                raise RuntimeError(
                    "WiFi v11 Mbps eval reached env done before the fixed-duration "
